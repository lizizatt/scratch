...
| [flight_train_claude](flight_train_claude/README.md) | Terminal IFR flight trainer (Cessna 208b Caravan) |
| [map_llm](map_llm/README.md) | Chat with a local LLM; log prompts and explore responses as a similarity graph |
| `fanime` | Static HTML/CSS prototypes — quest log and world-boss UI (see `quest_list_requirements.md`) |
...
Single-file terminal toys (no README): `blep` (Black Mage dodge game), `type` (centered text), `epiano` (FluidSynth keyboard), `flight-train` (WIP pygame trainer).
