...
describe("Jarvis deployment runner", () => {
  it("uses strict Vite startup and cleans up on SIGTERM", async () => {
    ...
    runner = spawn(runnerPath, [], {
      env: {
        ...
        JARVIS_JAM_ASSISTANT_ROOT: fixture.root,
        JARVIS_JAM_ASSISTANT_HOST: "127.0.0.2",
        JARVIS_JAM_ASSISTANT_PORT: "14173",
        JARVIS_JAM_ASSISTANT_TAILSCALE_PORT: "14174",
        JARVIS_JAM_ASSISTANT_READINESS_TIMEOUT_SECONDS: "2",
        JARVIS_JAM_ASSISTANT_TAILSCALE_EXECUTABLE: fixture.tailscalePath,
      },
      stdio: ["ignore", "ignore", "pipe"],
    });

    await waitForLog(fixture.logPath, "tailscale serve --bg --https=14174 http://127.0.0.2:14173");
    runner.kill("SIGTERM");
    expect(await waitForExit(runner)).toEqual({ code: 143, signal: null });

    const log = await readFile(fixture.logPath, "utf8");
    expect(log).toContain(`npm run dev -- --host 127.0.0.2 --port 14173 --strictPort cwd=${fixture.root}`);
    expect(log).toContain("curl http://127.0.0.2:14173/");
    expect(log).toContain("app SIGTERM");
    expect(log.split("\n").filter((line) => line === "tailscale serve --https=14174 off")).toHaveLength(2);
  });
});
...
async function createFixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), "jam-assistant-deployment-test-"));
