...
13. **Monster Hunt (Daisy) sim.** Guide `monster-hunts`: `character.s.monsterhunt={id,c,sn,ms}`; `interact("monsterhunt")` near Daisy (main 126,-413); merchants cannot accept; refuse while `c>0`; kills only on issuing `sn`; turn-in grants `monstertoken`. Sim + runner + `tests/test_monsterhunt.js` (3× goo/bee/crab cycle). Live fighter auto-quest not wired yet — scenario drives via existing `!hunt` intent.
