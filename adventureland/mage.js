...
var hold = !!ls(), last_pots_say = new Date(0), last_port_no = new Date(0), last_gold = new Date(0), seen = {};
...
function ding() { if (character.level <= last_lv) return; last_lv = character.level; party_say("Ding! Spark."); var e = []; for (var n in G.skills) if (G.skills[n].emote) e.push(n); if (e.length) use_skill(e[Math.floor(Math.random() * e.length)]); }
...
async function follow_lead() {
  ...
  if (p && (character.map !== p.map || parent.distance(character, p) > 180) && !smart.moving)
...
function at_pack() { farm = farm || desired(); return farmable(get_nearest_monster({ type: farm, max_att: farm_ovr ? 1e9 : att_cap() })); }
function port_no(why) { if (mssince(last_port_no) < 20000) return; last_port_no = new Date(); party_say(why); }
function try_port(name) {
  if (rally || busy || smart.moving) return port_no("Can't port (busy)");
  if (!at_pack()) return port_no("Can't port (not at pack)");
  if (!ready("magiport")) return port_no("Can't port (need " + ((G.skills.magiport || {}).mp || 900) + " MP)");
  party_say("Hang on!"); use_skill("magiport", name);
}
function combat() {
...
function hear(d) {
  ...
  else if (m.indexOf("summon") >= 0) try_port(d.from);
