...
function ding() {
  ...
  last_lv = character.level; party_say("Ding! Spark.");
...
function at_pack() { farm = farm || desired(); return farmable(get_nearest_monster({ type: farm, max_att: att_cap() })); }
...
function hear(d) {
  ...
  if (m.indexOf("summon") >= 0) try_port(d.from);
