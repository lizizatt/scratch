...
"use strict";
/**
 * Live restock prove: Jazwyn + Sarene + Puppygirl.
 * Jazwyn dumps all potions to Sarene, requests merchant restock,
 * and must finish with new pots (LIVE_RESTOCK_OK).
 */
const fs = require("fs");
...
async function main() {
  const deadlineMs = Number(process.env.LIVE_RESTOCK_MS || 18 * 60 * 1000);
  enableLiveTest(true);
  try {
    console.log("Deploy (LIVE_TEST warrior)...");
    ...
    await rpc("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "al-restock-trio", version: "1.0" },
    });
    ...
    console.log("log cut", new Date(cut).toISOString());
    for (const c of TRIO) {
      await tool("mainframe_link_character", {
        character: c,
        request_id: "restock-" + c + "-" + stamp,
        code_slot: SLOTS[c],
        server: "US III",
      });
    ...
    const before = potCounts((await tool("mainframe_get_character", { character: "Jazwyn" })).profile.inventory);
    console.log("Jazwyn pots before (snapshot)", before);

    const seen = {};
    const stats = { dump: 0, req: 0, doneOk: 0, restock: 0, restockHp: 0, restockMp: 0 };
    const deadline = Date.now() + deadlineMs;
    const interesting = /LIVE_|dlv:|Dump|Wait dlv|Dlv |send |buy |park |cycle |stock soft|Combine|Hunt |Form|party:|approach/i;
    ...
    while (Date.now() < deadline) {
      for (const c of TRIO) {
        ...
        for (const entry of raw.logs || []) {
          ...
          if (c === "Jazwyn" && /LIVE_DUMP_OK|LIVE_DUMP send /.test(s)) stats.dump++;
          if (c === "Jazwyn" && /Wait dlv/.test(s) && stats.dump < 1) stats.dump = 1;
          if ((c === "Jazwyn" || c === "Puppygirl") && /dlv:req /.test(s)) stats.req++;
          if (
            (c === "Jazwyn" && /LIVE_DLV_OK|dlv:got id=.* ok=1|dlv:done id=.* ok=1/.test(s)) ||
            (c === "Puppygirl" && /dlv:done id=.* ok=1|dlv:send /.test(s))
          )
            stats.doneOk++;
      ...
      const livePots = potCounts((await tool("mainframe_get_character", { character: "Jazwyn" })).profile.inventory);
      if (stats.dump >= 1 && stats.doneOk >= 1 && livePots.hp > 0 && livePots.mp > 0 && !stats.restock) {
        stats.restock = 1;
        stats.restockHp = livePots.hp;
        stats.restockMp = livePots.mp;
        console.log("infer restock from inventory", livePots);
      }
      console.log(
        "--- dump=%s req=%s doneOk=%s restock=%s hp=%s mp=%s inv=%s/%s ---",
        stats.dump,
        stats.req,
        stats.doneOk,
        stats.restock,
      ...
      if (stats.dump >= 1 && stats.restock && stats.restockHp > 0 && stats.restockMp > 0) break;
      await sleep(15000);
    }

    const afterCh = await tool("mainframe_get_character", { character: "Jazwyn" });
    const after = potCounts(afterCh.profile.inventory);
    const result = { stats, before, after, at: new Date().toISOString() };
    fs.writeFileSync(path.join(ROOT, "_live_restock_trio.json"), JSON.stringify(result, null, 2));
    console.log("result", JSON.stringify(result));

    if (!stats.dump) throw new Error("FAIL: no LIVE_DUMP_OK (Jazwyn never traded pots to Sarene)");
    if (!stats.req) throw new Error("FAIL: no dlv:req after dump");
    if (!stats.restock || !(stats.restockHp > 0 && stats.restockMp > 0)) {
      throw new Error("FAIL: Jazwyn did not restock potions (need LIVE_RESTOCK_OK with hp>0 mp>0)");
    }
    console.log("SUCCESS Jazwyn restocked hp=%s mp=%s", stats.restockHp, stats.restockMp);
  } finally {
    enableLiveTest(false);
    console.log("Restored warrior.js (LIVE_TEST off); redeploying...");
