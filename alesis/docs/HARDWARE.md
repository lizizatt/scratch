...
The metronome uses short woodblock notes from the isolated FluidR3 percussion layer on reserved channel 16. Recorded pads and generated drums use the same auxiliary kit on channel 10, so metronome note-offs cannot collide with recorded percussion. The metronome triggers exactly once per beat, accents each measure start, receives no selected-instrument chorus/reverb sends, and remains outside the take capture path. Its 25% default uses perceptual velocity scaling to sit between the earlier loud GM click and the softer resonant preset click.

Recorded loop playback preserves channel-10 drum events instead of remapping them onto melodic take channels. Melodic pitch bend is remapped to each take's playback channel and explicitly centered when a take is muted, stopped, or restarted at a cycle boundary.
